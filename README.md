...
4. 构建应用。脚本会自动覆盖 `/Applications/Awake Time.app` 并启动新版本：

   ```bash
   ./Scripts/build-app.sh
   ```

5. 如果只想在 `build/` 中生成应用而不安装，执行：
...
6. 在“设置…”→“通用”中设定习惯睡眠时长。夜晚切换点按 `(24 小时 - 睡眠时长) / 2` 计算，例如睡眠 8 小时会在醒来后 8 小时时切换月亮图标。
7. 睡觉提醒默认开启并提前 1 小时，可在同一区域按 15 分钟调整。提醒点按 `24 小时 - 睡眠时长 - 提前量` 计算，例如睡眠 8 小时、提前 1 小时会在醒来计时 `15:00` 时通知。
